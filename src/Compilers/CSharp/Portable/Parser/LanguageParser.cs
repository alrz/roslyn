...
tryAgain:
        ...
        private ScanTypeFlags ScanType(ParseTypeMode mode, out SyntaxToken lastTokenOfType)
        {
            ...
            while (IsMakingProgress(ref lastTokenPosition))
            {
                ...
                {
                    case SyntaxKind.QuestionToken
                            when lastTokenOfType.Kind is not (SyntaxKind.QuestionToken or // don't allow `Type??`
                                 SyntaxKind.AsteriskToken): // don't allow `Type*?`
                        lastTokenOfType = this.EatToken();
            ...
            if (tk == SyntaxKind.IdentifierToken)
            {
                    ...
                    token1.TrailingTrivia.Any((int)SyntaxKind.EndOfLineTrivia))
                {
                        ...
                        PeekToken(3).Kind == SyntaxKind.IdentifierToken)
                    {
                        ...
                        if (token4Kind is not (SyntaxKind.SemicolonToken or
        ...
        {
            if (this.CurrentToken.Kind is not (SyntaxKind.CloseParenToken or SyntaxKind.SemicolonToken))
            {
tryAgain:
...
tryAgain:
        ...
        private bool ShouldParseLambdaParameterType(bool hasModifier)
        {
            ...
            if (this.IsTrueIdentifier(this.CurrentToken))
            {
                ...
                if (peek1.Kind is not (SyntaxKind.CommaToken or

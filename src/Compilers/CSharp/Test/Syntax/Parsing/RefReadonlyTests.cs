...
class Test
{
    void M()
    {
        ...
        ref readonly int invalid = ref readonly value;
                ...
                // (8,54): error CS9000: Read-only variables must be initialized.
                //         ref readonly int invalid = ref readonly value;
                Diagnostic(ErrorCode.ERR_ReadOnlyVariableWithNoInitializer, ";").WithLocation(8, 54)
                );
        }
...
class Test
{
    void M()
    {
        ...
        ref int invalid = ref readonly value;
                ...
                // (8,45): error CS9000: Read-only variables must be initialized.
                //         ref int invalid = ref readonly value;
                Diagnostic(ErrorCode.ERR_ReadOnlyVariableWithNoInitializer, ";").WithLocation(8, 45)
                );
        }
...
class Test
{
    ...
    void N()
    {
        ...
        M(ref readonly x);
                ...
                // (10,25): error CS9000: Read-only variables must be initialized.
                //         M(ref readonly x);
                Diagnostic(ErrorCode.ERR_ReadOnlyVariableWithNoInitializer, ")").WithLocation(10, 25),
...
}").GetParseDiagnostics().Verify(); // Not a parse error
        }
...
class Test
{
    void M(ref readonly int p) => throw null;
                ...
                // (4,16): error CS9001: The parameter modifier 'readonly' cannot be used with 'ref'; use 'in' instead.
                //     void M(ref readonly int p) => throw null;
                Diagnostic(ErrorCode.ERR_ReadOnlyParameterWithRef, "readonly").WithLocation(4, 16)
                );
        }
    }
}
